 ...
 *
 *  - TWI clock speed has to be reduced to the limits defined in the table below.
 *    To avoid issues with an internal timer, speeds should be >= 50 KHz.
 *    Default mode is TWI_NAKED_ISR + release mode,
 *    thus you can safely run the TWI clock at up to 280 KHz.
 *
 *                      | TWI_DEBUG    | release mode |
 *    ------------------|--------------|--------------|
 *    normal interrupts | <= 120 KHz   | <= 130 KHz   |
 *    TWI_NAKED_ISR     | <= 250 KHz   | <= 280 KHz   |
 *
 *
