...
/* report descriptor:
 *      .  .  .  .  . S2 S1 S0 .... one byte with power signals
 * where:
 *     S2: system wakeup
 *     S1: system sleep
 *     S0: system power down
 */
typedef struct{
    uchar   powerSignalMask;
}report_t;
...
static void togglePowerDown(void)
{
	if(reportBuffer.powerSignalMask)
		reportBuffer.powerSignalMask = 0;
	else
		reportBuffer.powerSignalMask = (1 << 0);
}
